...
(function (global) {
  ...
  var CABLE_GROUPS = [
    { id: 'ftth', label: 'FTTH Main Cable', kind: 'distribution', capacities: [48, 72, 144, 288], defaultCap: 72 },
    { id: 'lastmile', label: 'Last Mile Cable', kind: 'lastmile', capacities: [12, 24, 36, 48], defaultCap: 12 },
  ];
  ...
  function getEntityLabelEntries() {
    return global.FTTHLabelDataProviders.getEntityLabelEntries();
  }

  /** Read-only cable labels at path midpoint for LabelManager overlay. */
  function getCableSegmentLabels() {
    return global.FTTHLabelDataProviders.getCableSegmentLabels();
  }
  ...
  function pointerToWorkspaceXY(clientX, clientY) {
    ...
    var zoom = Math.max(getMapCoordZoom(), 0.1);
    // Use the fixed viewport rect, then undo pan/zoom to get workspace coordinates.
    var localX = clientX - rect.left;
    var localY = clientY - rect.top;
    return {
      x: (localX - (Sim.panX || 0)) / zoom,
      y: (localY - (Sim.panY || 0)) / zoom,
    };
  }
  ...
  function applyCellLockPosition(el, node) {
    ...
    if (el.classList.contains('placed-node--top-layer') && node && Sim.layout) {
      ...
      var cs = Sim.layout.cellSize || 50;
      el.style.setProperty('left', (node.col * cs) + 'px', 'important');
      el.style.setProperty('top', (node.row * cs) + 'px', 'important');
  ...
  function pointerClientToCanvasXY(clientX, clientY) {
    ...
    return pointerClientToCanvasXYLegacy(clientX, clientY);
  }

  function pointerClientToCanvasXYLegacy(clientX, clientY) {
    return pointerToWorkspaceXY(clientX, clientY);
  }
  ...
  function isFakeFullscreen() {
    return document.body.classList.contains('ftth-fake-fullscreen-active');
  }
  ...
  function syncFullscreenButtonUi() {
    try {
      var isFs = isFakeFullscreen();
      var btn = document.getElementById('btn-fullscreen-map') || document.getElementById('btn-fullscreen');
      if (btn) {
        btn.textContent = isFs ? '⛶' : '🖥️';
        btn.title = isFs ? 'Exit expanded view' : 'Expand workspace';
  ...
  function toggleAppFullscreen() {
    try {
      ...
      var willExpand = !isFakeFullscreen();
      document.body.classList.toggle('ftth-fake-fullscreen-active', willExpand);
      workspace.classList.toggle('is-fullscreen', willExpand);
      ...
      console.error('Fake fullscreen toggle error:', error);
    }
  }
  ...
  function bindFullscreenButton() {
    var fsButton = document.getElementById('btn-fullscreen-map') || document.getElementById('btn-fullscreen');
    if (!fsButton || fsButton.dataset.ftthFsBound === '1') return;
    fsButton.dataset.ftthFsBound = '1';
    fsButton.addEventListener('click', toggleAppFullscreen);
  }
  ...
  function handleShortcuts(e) {
    ...
    if (isCtrlUndo(e)) {
      ...
      e.preventDefault();
      if (hasActiveDrawingStroke() && global.FTTHDrawingEngine?.handlePenKeyDown?.(e)) {
        return;
      }
      var undoBtn = document.getElementById('btn-undo');
  ...
  function canVertexEdit() {
    return getActiveCanvasTool() === 'vertex';
  }
  ...
  function getActiveCanvasTool() {
    if (Sim.pathEdit?.vertexToolActive || Sim.activeCanvasTool === 'vertex') return 'vertex';
    if (Sim.pathEdit?.splitToolActive || Sim.activeCanvasTool === 'cut') return 'cut';
    return Sim.activeCanvasTool || 'select';
  }

  function setActiveCanvasTool(tool) {
    var next = (tool === 'vertex') ? 'vertex' : (tool === 'cut' || tool === 'select' ? tool : 'select');
    Sim.activeCanvasTool = next;
    if (!Sim.pathEdit) {
      Sim.pathEdit = {
        drag: null, context: null, editActive: false,
        splitToolActive: false, vertexToolActive: false,
    ...
    Sim.pathEdit.vertexToolActive = next === 'vertex';
    Sim.pathEdit.splitToolActive = next === 'cut';
    Sim.pathEdit.cutHover = null;
    Sim.pathEdit.cutSwipe = null;
    if (next === 'vertex') {
      if (!Sim.selectedPath) {
        ...
        updateStatus('Vertex Tool — select a cable path first', true);
        next = 'select';
      } else {
    ...
    if (next === 'select') {
      Sim.crosshair = null;
      Sim.pathEdit.vertexToolActive = false;
      if (!Sim.pathEdit.splitToolActive) Sim.pathEdit.editActive = !!isPathPropertiesPanelOpen();
    }
    if (global.FTTHDrawingEngine?.syncCanvasToolChrome) {
      global.FTTHDrawingEngine.syncCanvasToolChrome();
    }
    if (global.FTTHDrawingEngine?.hideCrosshair && (next === 'select' || Sim.interactionMode === 'hand')) {
    ...
    renderGlobalDrawingLayer();
    if (next === 'vertex') {
      updateStatus('Vertex Tool — drag vertices on map · double-click path to add point');
    } else if (next === 'cut') {
      updateStatus('Cut — swipe through path or click segment');
    } else {
  ...
  function isDrawingLayerInteractive() {
    ...
    if (tool === 'vertex' || tool === 'cut') return true;
  ...
  function appendCableConfiguratorPanel(box, parent, grp) {
    ...
    if (grp.kind === 'lastmile') {
      ...
      selectRow.className = 'cable-config-panel__select-row';
      var select = document.createElement('select');
      select.className = 'fiber-dropdown toolbox-card__select cable-config-panel__select cable-glow-select';
      select.setAttribute('aria-label', grp.label + ' capacity');
      grp.capacities.forEach(function (cap) {
        var opt = document.createElement('option');
        opt.value = String(cap);
        opt.textContent = cap + 'F';
        if (cap === cfg.capacity) opt.selected = true;
      ...
      select.addEventListener('change', function (e) {
        e.stopPropagation();
        var cap = parseInt(select.value, 10);
        setCableCapacity(grp.kind, cap, box);
        armPenCable(grp, cap, box, panel);
      });
  ...
  function renderGlobalDrawingLayer() {
    ...
    if (vtxRef && !canPenDraw() && canVertexEdit()) {
  ...
  function fatPoleMapGlyph() {
    ...
    return '<svg class="field-glyph field-glyph--fat-pole fat-pole-glyph" viewBox="0 0 ' + FAT_POLE_GLYPH_W + ' ' + FAT_POLE_GLYPH_H + '" width="' + FAT_POLE_GLYPH_W + '" height="' + FAT_POLE_GLYPH_H + '" aria-hidden="true">' +
      '<path class="fat-pole-path" d="' + d + '" fill="none" stroke="#808080" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="miter" vector-effect="non-scaling-stroke"/>' +
      '</svg>';
  }
  ...
  function fatPoleHtml(node) {
    ...
    return '<div class="fat-pole" aria-hidden="true">' + fatPoleMapGlyph() + boxInner + '</div>';
  }
  ...
  function fatSystemHtml(node, size) {
    ...
    return '<div class="' + anchorCls + '">' +
      ...
      (node.hasFatPole ? fatPoleHtml(node) : '') +
      '</div>';
  }
  ...
  function poleSquareLabelTopPx(node, anchorEl) {
    var anchorH = anchorEl && anchorEl.offsetHeight > 0 ? anchorEl.offsetHeight : iconSizeForStackLayer(node);
  ...
  function attachNodeLabels(placedEl, node) {
    ...
    getNodeLabelEntries(node).forEach(function (entry) {
      ...
      lbl.addEventListener('mousedown', function (e) {
        ...
        e.stopPropagation();
        e.preventDefault();
        startElementLabelDrag(node, entry.key, lbl, anchor, e);
      });
    });
  }
  ...
  function renderNode(node) {
    ...
    if (node.type === 'fat_handhole' && node.hasFatPole) m.classList.add('placed-node--fat-pole');
  ...
  function renderSidebarHeader() {
    var editOn = !!Sim.ui.sidebarEditMode;
    return '<div class="unified-sidebar__head">' +
      '<p class="unified-sidebar__title">Features &amp; Properties</p>' +
      '<button type="button" class="unified-sidebar__edit-btn' + (editOn ? ' unified-sidebar__edit-btn--active' : '') +
      '" data-sidebar-action="toggle-edit" title="Edit mode" aria-label="Toggle edit mode" aria-pressed="' + (editOn ? 'true' : 'false') + '">' +
      '<svg class="unified-sidebar__edit-icon" viewBox="0 0 24 24" width="16" height="16" aria-hidden="true">' +
      '<rect x="4" y="4" width="16" height="16" rx="2" fill="none" stroke="currentColor" stroke-width="1.75" stroke-dasharray="3 2"/>' +
      '<path d="M8 15l8-8M13 7l4 4" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round"/>' +
      '</svg></button></div>';
  }

  function renderPathSectionTools(pathType, pathId) {
    if (!Sim.ui.sidebarEditMode || !pathType || !pathId) return '';
    var editing = Sim.pathEdit?.editingPathId;
    var isFocus = editing && editing.type === pathType && editing.id === pathId;
    var cutOn = isFocus && !!(Sim.pathEdit?.splitToolActive || getActiveCanvasTool() === 'cut');
    var vtxOn = isFocus && !!(Sim.pathEdit?.vertexToolActive || getActiveCanvasTool() === 'vertex');
    return '<div class="path-section-tools" data-path-section="' + escapeSidebarHtml(pathType + ':' + pathId) + '">' +
      '<button type="button" class="path-section-tools__btn' + (vtxOn ? ' path-section-tools__btn--active' : '') +
      '" data-sidebar-action="vertex" data-path-type="' + escapeSidebarHtml(pathType) +
      '" data-path-id="' + escapeSidebarHtml(pathId) + '" title="Move vertices">🎯</button>' +
      '<button type="button" class="path-section-tools__btn' + (cutOn ? ' path-section-tools__btn--active' : '') +
      '" data-sidebar-action="cut" data-path-type="' + escapeSidebarHtml(pathType) +
      '" data-path-id="' + escapeSidebarHtml(pathId) + '" title="Cut path">✂️</button>' +
      '<button type="button" class="path-section-tools__btn path-section-tools__btn--danger" data-sidebar-action="delete" data-path-type="' +
      escapeSidebarHtml(pathType) + '" data-path-id="' + escapeSidebarHtml(pathId) + '" title="Delete">🗑️</button>' +
      '</div>';
  }
  ...
  function renderNodeSectionTools(nodeId) {
    ...
    return '<div class="path-section-tools sidebar-node-tools" data-path-section="' + escapeSidebarHtml('node:' + nodeId) + '">' +
      ...
      (node.locked ? '🔒' : '🔓') + '</button>' +
      '<button type="button" class="path-section-tools__btn path-section-tools__btn--danger" data-sidebar-action="delete-node" data-focus-node-id="' +
      escapeSidebarHtml(nodeId) + '" title="Delete">🗑️</button>' +
      '</div>';
  }
  ...
  function renderSidebarSection(title, cardsHtml, emptyText, modifier) {
    var body = cardsHtml ||
    ...
    return '<section class="sidebar-section' + modCls + '">' +
      '<h3 class="sidebar-section__title">' + escapeSidebarHtml(title) + '</h3>' +
      '<ul class="sidebar-section__cards">' + body + '</ul>' +
      '</section>';
  }
  ...
  function renderThreeSectionSidebar(node, focus) {
    ...
    var holesHtml = renderHandholeSidebarCard(node, focus.kind === 'node');
    ...
    return '<div class="sidebar-sections-view">' +
      ...
      renderSidebarSection('AB_LM_Excavation', excavHtml, 'No excavation', 'excav') +
      renderSidebarSection('UnderGround FTTH_Cable', cableHtml, 'No cables', 'cable') +
      '</div>';
  }
  ...
  function renderPathPropertyContent() {
    ...
    var path = meta.path;
    var anchorNode = resolvePrimaryHandholeForPath(Sim.selectedPath.type, path);
    if (anchorNode) {
      if (!Sim.ui.topologyTreeFocus) {
        Sim.ui.topologyTreeFocus = {
  ...
  function renderUnifiedSidebar() {
    ...
    if (Sim.selectedPath) html += renderPathPropertyContent();
    else if (Sim.selectedNodeId) html += renderNodePropertyContent(findNode(Sim.selectedNodeId));
    body.innerHTML = html;
  ...
  function setSmartStatusHint(msg) {
    var el = document.getElementById('smart-status-hint');
    if (el && msg) el.textContent = msg;
  }

  function syncSmartStatusBarTools() {
    var isHand = Sim.interactionMode === 'hand';
    var selBtn = document.getElementById('status-btn-select');
    var handBtn = document.getElementById('status-btn-hand');
    if (selBtn) {
      selBtn.classList.toggle('smart-status-bar__btn--active', !isHand);
      selBtn.setAttribute('aria-pressed', !isHand ? 'true' : 'false');
    }
    if (handBtn) {
      handBtn.classList.toggle('smart-status-bar__btn--active', isHand);
      handBtn.setAttribute('aria-pressed', isHand ? 'true' : 'false');
    }
  }

  function bindSmartStatusBar() {
    if (Sim.ui.smartStatusBarBound) return;
    var selBtn = document.getElementById('status-btn-select');
    var handBtn = document.getElementById('status-btn-hand');
    var zoomBtn = document.getElementById('status-btn-zoom-reset');
    var mapSelect = document.getElementById('tool-select');
    var mapHand = document.getElementById('tool-hand');
    if (selBtn && mapSelect) {
      selBtn.addEventListener('click', function () { mapSelect.click(); });
    }
    if (handBtn && mapHand) {
      handBtn.addEventListener('click', function () { mapHand.click(); });
    }
    if (zoomBtn) {
      zoomBtn.addEventListener('click', function () {
        var reset = document.getElementById('btn-zoom-reset');
        if (reset) reset.click();
      });
    }
    Sim.ui.smartStatusBarBound = true;
  ...
  function updateFieldStatusCounters() {
    ...
    summaryEl.textContent = '[ Active Batch: ' + getActiveBatchLabel() +
      ' | Last Element: ' + getLastInstalledElementLabel() + ' ]';
  }
  ...
  function syncPanelToggleArrows() {
    ...
    if (tbBtn) tbBtn.textContent = Sim.ui.toolboxCollapsed ? '▶' : '◀';
  ...
  function bindPanelToggles() {
    ...
    if (tbBtn && toolbox) {
      tbBtn.addEventListener('click', function (e) {
        e.preventDefault();
        e.stopPropagation();
  ...
  function setMapRotation(deg) {
    Sim.mapRotation = ((deg % 360) + 360) % 360;
    ...
    if (rotSlider) rotSlider.value = String(Math.round(Sim.mapRotation));
    if (rotVal) rotVal.textContent = Math.round(Sim.mapRotation) + '°';
    applyMapTransform();
  }

  function applyMapContainerTransform(container, wrap, panX, panY, z, rot) {
    if (!container) return;
    if (rot && wrap) {
      ...
      wrap.classList.add('map-rotation-active');
    } else {
      container.style.transformOrigin = '0 0';
      container.style.transform =
        'translate(' + panX + 'px,' + panY + 'px) scale(' + z + ')';
      if (wrap) wrap.classList.remove('map-rotation-active');
    }
  }

  function flushMapViewportTransform() {
    ...
    var panX = Sim.panX || 0;
    var panY = Sim.panY || 0;
    var rot = Sim.settings.mapRotationEnabled ? (Sim.mapRotation || 0) : 0;
    ...
    applyMapContainerTransform(container, wrap, panX, panY, z, rot);
  ...
  function setZoom(val, pivotX, pivotY, clientX, clientY, opts) {
    ...
    var newZoom = Math.max(MAP_ZOOM_MIN, Math.min(MAP_ZOOM_MAX, Math.round(val * 1000) / 1000));
    if (Math.abs(newZoom - oldZoom) < 1e-6) return;

    var DE = global.FTTHDrawingEngine;
    if (DE?.computeZoomPanAdjust && clientX != null && clientY != null) {
      var adj = DE.computeZoomPanAdjust(
        oldZoom, newZoom, Sim.panX || 0, Sim.panY || 0, clientX, clientY
      );
      Sim.panX = adj.panX;
      Sim.panY = adj.panY;
    } else if (pivotX != null && pivotY != null) {
      var ratio = newZoom / oldZoom;
      Sim.panX = pivotX - (pivotX - (Sim.panX || 0)) * ratio;
      Sim.panY = pivotY - (pivotY - (Sim.panY || 0)) * ratio;
    }
    Sim.zoom = newZoom;
    applyMapTransform({ skipRedraw: !!opts.light });
  ...
  function onToolSelectClick() {
    setInteractionMode('select');
    if (getActiveCanvasTool() !== 'vertex' && getActiveCanvasTool() !== 'cut') {
      setActiveCanvasTool('select');
    }
  }
  ...
  function onMapGesturePointerMove(e) {
    ...
    if (dx || dy) {
      ...
      session.panX += dx;
      session.panY += dy;
      Sim.panX = session.panX;
      Sim.panY = session.panY;
  ...
  function onCanvasWrapPointerLeave() {
  ...
  function applyInteractionModeClasses() {
    ...
    if (toolSelect) {
      toolSelect.classList.toggle('map-tool-btn--active', !isHand);
      toolSelect.setAttribute('aria-pressed', !isHand ? 'true' : 'false');
    }
    if (toolHand) {
      toolHand.classList.toggle('map-tool-btn--active', isHand);
  ...
  function setInteractionMode(mode) {
    var next = mode === 'hand' ? 'hand' : 'select';
    if (Sim.interactionMode === next && Sim.currentTool === next) {
      applyInteractionModeClasses();
      syncDrawingLayerInteraction();
      return;
    }
    resetMapPointerState({ keepCrosshair: next === 'select' });
    Sim.interactionMode = next;
    Sim.currentTool = next;
    applyInteractionModeClasses();
    syncDrawingLayerInteraction();
    global.FTTHDrawingEngine?.syncMapInteractionCursor?.();
    if (next === 'hand') {
      ...
      global.FTTHDrawingEngine?.hideCrosshair?.();
      global.FTTHDrawingEngine?.requestOverlayRedraw?.();
    }
  }
  ...
  function onPanStart(e) {
    ...
    var isHandPan = Sim.interactionMode === 'hand' && e.button === 0;
    var isMiddlePan = e.button === 1;
    var isSpacePan = e.button === 0 && !!Sim.ui?.spacePanActive;

    if (!isHandPan && !isMiddlePan && !isSpacePan) return;
  ...
  function onPanMove(e) {
    ...
    Sim.panX = Sim.panSession.panX + dx;
    Sim.panY = Sim.panSession.panY + dy;
    applyMapTransform();
  }
  ...
  function bindWheelZoom() {
    ...
    wrap.addEventListener('wheel', function (e) {
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        factor = delta > 0 ? 0.92 : 1.08;
      } else if (e.deltaMode === 2) {
        factor = delta > 0 ? 0.85 : 1.15;
      } else {
        factor = Math.exp(-delta * 0.002);
      }
      ...
      setZoom(Sim.zoom * factor, pivotX, pivotY, e.clientX, e.clientY);
    }, { passive: false });
  ...
  function bindSettingsPanel() {
    ...
    if (cableZoomSlider) {
      cableZoomSlider.value = String(Sim.settings.cableLabelZoomMin || 150);
      function onCableZoomChange() {
        Sim.settings.cableLabelZoomMin = clampLabelZoomPct(cableZoomSlider.value, 150);
        applyLabelZoomVisibility();
      }
    ...
    if (rotSlider) {
      rotSlider.addEventListener('input', function () {
        setMapRotation(parseInt(rotSlider.value, 10) || 0);
      });
    }
    ...
    if (rotLeft) {
      rotLeft.addEventListener('click', function () {
        ...
        setMapRotation((Sim.mapRotation || 0) - 15);
      });
    }
  ...
  function bindControls() {
    ...
    if (zi) zi.addEventListener('click', function () {
      setZoom(Sim.zoom + 0.1, wrap ? wrap.clientWidth / 2 : null, wrap ? wrap.clientHeight / 2 : null);
    });
    if (zo) zo.addEventListener('click', function () {
      setZoom(Sim.zoom - 0.1, wrap ? wrap.clientWidth / 2 : null, wrap ? wrap.clientHeight / 2 : null);
    });
    if (zr) zr.addEventListener('click', function () {
      Sim.zoom = 1; Sim.panX = 0; Sim.panY = 0;
      applyMapTransform();
    });
    ...
    syncFullscreenButtonUi();
    window.addEventListener('resize', function () { requestAnimationFrame(positionNodeActionHud); });
  }
    ...
    try {
      ...
      if (global.FTTHLabelManager?.init) {
        global.FTTHLabelManager.init({
          ...
          getCableLabelZoomMinPct: function () { return Sim.settings?.cableLabelZoomMin || 150; },
        });
      }
    } catch (err) {
